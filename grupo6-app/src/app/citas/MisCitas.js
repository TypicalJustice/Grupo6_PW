...
export default function App() {
  ...
  function handleSearch(parametro) {
    ...
    // Se hace la llamada al endpoint utilizando el valor del parámetro de búsqueda
    fetch(`http://localhost:3000/usuarios/${parametro}`)
  ...
  function handleDateSelect(dayOfWeek) {
    fetch(`http://localhost:3000/profesores/horario/${dayOfWeek}`)
      .then(response => response.json())
      .then(data => {
        setSearchResults(data);
      })
